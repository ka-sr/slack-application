...
app.get("/invitations/:email", async (req, res) => {
  try {
...
app.post("/accept-invite", async (req, res) => {
  try {
    ...
    if (
      !workspace.members.includes(user.name)
    ) {
      workspace.members.push(user.name);
    }
